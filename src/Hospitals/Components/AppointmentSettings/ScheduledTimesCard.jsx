...
const ScheduledTimesCard = ({ data }) => {
    const { doctor, appointmentBy, setAppointmentBy } = useContext(
        AppointmentSettingContext
    );

    return (
        <div
            className={`relative flex gap-[25px] p-4 pt-6 leading-[18px] font-f3 font-w1 text-[15px] ml-2 border border-c17 w-fit rounded-[5px] ${doctor?.acceptAppointments == "byToken" && appointmentBy != "token" ? "bg-[#D9D9D961] text-[#706D6D]" : doctor?.acceptAppointments == "bySlot" && appointmentBy != "slot" ? "bg-[#D9D9D961] text-[#706D6D] " : "text-c20"}`}
        >
